...
function buildA4Html(doc: BillingDocumentView, locale: string) {
  return [
    ...
    '<p class="muted">' + escapeHtml(doc.sellerAddress || '-') + '</p>',
    '<p class="muted">Tax ID: ' + escapeHtml(doc.sellerTaxId || '-') + '</p>',
    '</section>',
    '<section class="doc-box">',
    '<div class="doc-kind">' + (doc.docKind === 'receipt' ? 'Receipt' : 'Invoice') + '</div>',
    '<div>Document No: ' + escapeHtml(doc.documentNo || '-') + '</div>',
    '<div>Reference: ' + escapeHtml(doc.referenceNo || '-') + '</div>',
    '<div>Date: ' + escapeHtml(formatDate(doc.issueDate, locale)) + '</div>',
    doc.docKind === 'invoice' ? '<div>Due: ' + escapeHtml(formatDate(doc.dueDate, locale)) + '</div>' : '',
    '</section>',
    '</div>',
    ...
    '<div class="muted">' + escapeHtml(doc.buyerAddress || '-') + '</div>',
    '<div class="muted">Tax ID: ' + escapeHtml(doc.buyerTaxId || '-') + '</div>',
    '</section>',
    '<section>',
    '<div class="label">Contact</div>',
    '<div class="value">' + escapeHtml(doc.contactName || '-') + '</div>',
    '<div class="muted">' + escapeHtml(doc.contactPhone || '-') + '</div>',
    '</section>',
    '</div>',
    '<table class="items">',
    ...
    '<div><span>Subtotal</span><span>' + escapeHtml(formatCurrency(doc.subtotal, locale)) + ' ' + escapeHtml(doc.currency) + '</span></div>',
    '<div><span>Discount ' + escapeHtml(String(doc.discountPercent)) + '%</span><span>-' + escapeHtml(formatCurrency(doc.discountAmount, locale)) + ' ' + escapeHtml(doc.currency) + '</span></div>',
    ...
    '<div class="grand"><span>Grand Total</span><span>' + escapeHtml(formatCurrency(doc.grandTotal, locale)) + ' ' + escapeHtml(doc.currency) + '</span></div>',
    '</section>',
    '<section class="footer-note">',
    '<div>Payment: ' + escapeHtml(doc.paymentMethod || '-') + '</div>',
    '<div>Note: ' + escapeHtml(doc.noteMessage || '-') + '</div>',
    '</section>',
    '</article>',
  ].join('');
}
...
function buildThermalHtml(doc: BillingDocumentView, locale: string) {
  return [
    ...
    '<div class="center">' + escapeHtml(doc.contactPhone || '-') + '</div>',
    '<div class="center strong">' + (doc.docKind === 'receipt' ? 'ใบเสร็จ' : 'ใบแจ้งหนี้') + '</div>',
    '<hr />',
    '<div>เลขที่: ' + escapeHtml(doc.documentNo || '-') + '</div>',
    '<div>อ้างอิง: ' + escapeHtml(doc.referenceNo || '-') + '</div>',
    '<div>วันที่: ' + escapeHtml(formatDate(doc.issueDate, locale)) + '</div>',
    doc.docKind === 'invoice' ? '<div>ครบกำหนด: ' + escapeHtml(formatDate(doc.dueDate, locale)) + '</div>' : '',
    ...
    '<div class="r">ส่วนลด ' + escapeHtml(String(doc.discountPercent)) + '%: -' + escapeHtml(formatCurrency(doc.discountAmount, locale)) + '</div>',
    '<div class="r">VAT ' + escapeHtml(String(doc.vatPercent)) + '%: ' + escapeHtml(formatCurrency(doc.vatAmount, locale)) + '</div>',
    '<div class="r strong">ยอดสุทธิ: ' + escapeHtml(formatCurrency(doc.grandTotal, locale)) + ' บาท</div>',
    '<hr />',
    '<div class="center">ชำระโดย: ' + escapeHtml(doc.paymentMethod || '-') + '</div>',
    '<div class="center">' + escapeHtml(doc.noteMessage || '*** ขอบคุณที่ใช้บริการ ***') + '</div>',
    '</article>',
  ].join('');
}
...
export function buildBillingExportHtml(input: {
  ...
  return [
    ...
    '</div>',
    body,
    input.print ? '<script>setTimeout(function(){ window.print(); }, 180);</script>' : '',
    '</body>',
    '</html>',
  ].join('');
}
