...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  void enqueuePushNotification({
    ...
    if (!queued.ok) return;
    void processPushQueue({ batchSize: 10 }).catch(function (queueError) {
      console.error("Push process after vault secret action failed:", queueError);
    });
  }).catch(function (queueError) {
    console.error("Push enqueue for vault secret action failed:", queueError);
  });
