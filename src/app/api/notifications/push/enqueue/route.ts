...
export async function POST(req: Request) {  
  ...
  const shouldProcessNow = body.processNow !== false;  
  if (shouldProcessNow) {  
    const processSummary = await processPushQueue({ batchSize: 20 });  
    return NextResponse.json({ ok: true, queuedId: queued.id, processSummary: processSummary });  
  }  
  
  return NextResponse.json({ ok: true, queuedId: queued.id });  
} 
