...
import { verifyPackageSlipWithProvider } from "@/lib/payment-slip-provider";
...
export async function POST(req: Request) {
  ...
  const timeMatched = transferredAt ? withinMinutes(order.created_at, transferredAt, 60) : true;
  const matched = amountMatched && receiverMatched && timeMatched;
  ...
  if (!timeMatched) noteParts.push("time_mismatch");
